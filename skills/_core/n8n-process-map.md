...
## 7. NestJS Server 调用的 n8n 接口

> 来源：coros-mes-prod 代码分析 | 2026-04-15

| n8n路径 | NestJS调用方 | 业务说明 | 迁移状态 |
|---------|-------------|----------|----------|
| /getproductmodel_API | ProductService | 获取产品模型 | 🔄待迁移(P1) |
| /getproductinfo_API | ProductService | 搜索产品信息 | 🔄待迁移(P2) |
| /createproduct_API | ProductService | 创建产品 | 🔄待迁移(P2) |
| /createproductmodel_API | ProductService | 创建产品模型 | 🔄待迁移(P1) |
| /getprocedure_API | OperationService, PackageService | 获取工序列表 | 🔄待迁移(P0,已有替代) |
| /createprocedure_API | OperationService | 创建工序 | 🔄待迁移(P3) |
| /updateprocedure_API | OperationService | 更新工序 | 🔄待迁移(P3) |
| /deleteprocedure_API | OperationService | 删除工序 | 🔄待迁移(P1) |
| /downloadprocedure_API | OperationService | 下载工序SOP | 🔄待迁移(P3) |
| /get_fqc | FQCService | FQC数据查询 | 🔄待迁移(P3) |
| /getPackageInfo | PackageService | 包装数据查询 | 🔄待迁移(P2) |
| /get_repair_message | RepairService | 维修记录查询 | 🔄待迁移(P3) |
| /select_common_api | HostComputerService | 通用上位机查询 | ✅已有替代(getHostComputerCommons) |
| /uuidGetSn | SearchService, HostComputerService | UUID转SN | ✅已有替代(uuidGetSn方法) |
